...
class FourSquareDataset:
    ...
    def construct_set(self, train_por, test_por, window_size, label=0):
        grouped = self.raw_data.groupby('userId')
        ...
        for user, traj in grouped:
            traj = traj.sort_values('utcTimestamp')[label].values
            if len(traj) < window_size + 1: continue
            len_train = int(len(traj) * train_por)
            train_traj = traj[:len_train]
            for i in range(len(train_traj) - window_size):
                train_x.append(train_traj[i:i + window_size])
                train_y.append(train_traj[i + window_size])
            
            len_val = int(len(traj) * (train_por + test_por))
            val_traj = traj[len_train:len_val]
            for i in range(len(val_traj) - window_size):
                val_x.append(val_traj[i:i + window_size])
                val_y.append(val_traj[i + window_size])
            
            test_traj = traj[len_val:]
            for i in range(len(test_traj) - window_size):
                test_x.append(test_traj[i:i + window_size])
                test_y.append(test_traj[i + window_size])
